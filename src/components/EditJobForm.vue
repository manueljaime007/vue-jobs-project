...
onMounted(async () => {
  try {
    ...
    const updatedJob = Object.assign(form, state.job);
  } catch (error) {
...
const updateJob = async () => {
  try {
    await axios.patch(`/api/jobs/${jobId}`, updateJob)
      ...
      <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-4">
            ...
            <input type="text" id="name" name="name" v-model="form.title" class="border rounded w-full py-2 px-3 mb-2"
          ...
          <div class="mb-4">
            ...
            <textarea id="company_description" name="company_description" class="border rounded w-full py-2 px-3"
              rows="4" placeholder="What does your company do?" v-model="form.company.description" />
          </div>
